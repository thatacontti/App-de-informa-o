...
function render(){
  ...
  document.getElementById('kpis').innerHTML=`<div class="kpi"><div class="l">Faturamento</div><div class="v">${brl(fat)}</div></div><div class="kpi"><div class="l">Peças</div><div class="v">${num(qtd)}</div><div class="s">PM ${brl(pm)}</div></div><div class="kpi"><div class="l">SKUs</div><div class="v">${sku}</div></div><div class="kpi"><div class="l">Clientes</div><div class="v">${cli}</div></div><div class="kpi"><div class="l">Pedidos</div><div class="v">${new Set(X.map(r=>r.p+'|'+r.c)).size}</div></div>`;
  // PARTICIPAÇÃO POR MARCA
  const mkShare={};let mkTot=0;
  X.forEach(r=>{mkShare[r.m]=(mkShare[r.m]||0)+r.f;mkTot+=r.f});
  ...
  let macroHTML=`<div class="heroSSS"><div class="hcard"><div class="hh">V26 · ${clisRec.length} cli recorrentes</div><div class="hv">${brl(macroV26)}</div></div><div class="harr">→</div><div class="hcard v27"><div class="hh">V27 dos mesmos ${clisRec.length} cli</div><div class="hv">${brl(macroV27)}</div></div><div class="harr">=</div><div class="hcard var"><div class="hh">SSS YoY</div><div class="hv gr">${vSSS>=0?'+':''}${vSSS.toFixed(1)}%</div><div class="hh" style="margin-top:6px">+ ${brl(macroNovoF)} de ${macroNovoCli.size} NOVO 27</div><div class="hh" style="margin-top:4px;opacity:.8">Carteira total V27: <b>${brl(f27Tot)}</b></div></div></div>`;
  // Linha de análise normalizada
  if(outliers.length>0){
    const sssNcl=sssNorm>=0?'gr':'rd';
    macroHTML+=`<div class="sss-norm"><div class="sss-norm-hdr"><b>Análise normalizada</b> · excluindo ${outliers.length} cliente${outliers.length>1?'s':''} com crescimento &gt; +100% (outliers)</div><div class="sss-norm-grid"><div class="snc"><div class="snc-l">${cliNormais.length} cli normais</div><div class="snc-v">V26 ${brl(v26Norm)} → V27 ${brl(v27Norm)}</div><div class="snc-r ${sssNcl}">SSS ${sssNorm>=0?'+':''}${sssNorm.toFixed(1)}%</div></div><div class="snc snc-out"><div class="snc-l">${outliers.length} outlier${outliers.length>1?'s':''} (&gt;+100%)</div><div class="snc-v">V26 ${brl(v26Out)} → V27 ${brl(v27Out)}</div><div class="snc-r gr">+${v26Out?((v27Out-v26Out)/v26Out*100).toFixed(0):0}%</div></div></div><div class="sss-norm-foot">Os outliers contribuíram com <b>${brl(v27Out-v26Out)}</b> de crescimento absoluto · <b>${(((v27Out-v26Out)/(macroV27-macroV26))*100).toFixed(0)}%</b> do delta YoY total veio deles</div></div>`;
  }
  document.getElementById('sss-macro').innerHTML=macroHTML;
  ...
  let ph='<table class="m"><thead><tr><th>Perfil</th><th class="r">Cli</th><th class="r">V26</th><th class="r">V27</th><th class="r">Var %</th><th class="r">Tk médio V27</th></tr></thead><tbody>';
  ORD.filter(p=>pm_[p]).forEach(p=>{const v=pm_[p];const vr=v.f26?(v.f27-v.f26)/v.f26*100:null;const cl=v.f26?(vr>0?'vu':'vd'):'nv';ph+=`<tr><td><span class="ppill cl-${pcX(p)}">${p}</span></td><td class="num">${v.n}</td><td class="money">${v.f26?brl(v.f26):'—'}</td><td class="money"><b>${brl(v.f27)}</b></td><td class="num ${cl}"><b>${vr!==null?(vr>=0?'+':'')+vr.toFixed(0)+'%':'NOVO'}</b></td><td class="money">${brl(v.f27/v.n)}</td></tr>`});
  document.getElementById('sss-perf').innerHTML=ph+'</tbody></table>';

  // Top clientes
  cArr.sort((a,b)=>b.f27-a.f27);
  let ch='<table class="m"><thead><tr><th>#</th><th>Cliente</th><th>Cidade</th><th>UF</th><th>Perfil</th><th class="r">V26</th><th class="r">V27</th><th class="r">Var %</th></tr></thead><tbody>';
  cArr.slice(0,20).forEach((c,i)=>{const vr=c.f26?(c.f27-c.f26)/c.f26*100:null;const cl=c.f26?(vr>20?'vu':vr<-20?'vd':'ve'):'nv';ch+=`<tr><td class="rk">#${i+1}</td><td><b>${c.nm.substring(0,28)}</b></td><td>${c.cid.substring(0,16)}</td><td>${c.uf}</td><td><span class="ppill cl-${pcX(c.pf)}">${c.pf}</span></td><td class="money">${c.f26?brl(c.f26):'—'}</td><td class="money"><b>${brl(c.f27)}</b></td><td class="num ${cl}"><b>${vr!==null?(vr>=0?'+':'')+vr.toFixed(0)+'%':'NOVO'}</b></td></tr>`});
  ...
  sssDynamic.forEach(s=>{const cl=s.var>0?'vu':'vd';sh+=`<tr><td><span class="pill pl-${mcls(s.DESC_MARCA)}">${s.DESC_MARCA}</span></td><td class="money">${brl(s.fat26)}</td><td class="money"><b>${brl(s.fat27)}</b></td><td class="num ${cl}"><b>${s.var>=0?'+':''}${s.var}%</b></td></tr>`});
  document.getElementById('sss-mca').innerHTML=sh+'</tbody></table>';
  ...
  // Render tabela
  let mlH='<table class="m"><thead><tr><th>Marca</th><th class="r">V26</th><th class="r">V27 total</th><th class="r">SSS YoY</th>';
  ...
  MARCAS_ORD.forEach(m=>{
    ...
    mlH+='<td class="money">'+brl(d.f26)+'</td>';
    mlH+='<td class="money"><b>'+brl(d.f27)+'</b></td>';
    mlH+='<td class="num '+cl+'"><b>'+(d.sss>=0?'+':'')+d.sss.toFixed(1)+'%</b></td>';
    LINHAS_ORD.forEach(l=>{
      const v=d.linhas[l]||0;
      const pct=d.f27?v/d.f27*100:0;
      mlH+='<td class="money">'+(v>0?brl(v)+'<br><span style="font-size:.6rem;color:#8a7e72">'+pct.toFixed(0)+'%</span>':'<span style="font-size:.6rem;color:#c9a080">—</span>')+'</td>';
    });
    mlH+='</tr>';
  });
  // Linha totais
  ...
  mlH+='<td class="money">'+brl(f26Tot)+'</td><td class="money"><b>'+brl(f27Tot2)+'</b></td>';
  mlH+='<td class="num '+clTot+'"><b>'+(sssTot>=0?'+':'')+sssTot.toFixed(1)+'%</b></td>';
  LINHAS_ORD.forEach(l=>{
    const v=linhaTotals[l]||0;const pct=f27Tot2?v/f27Tot2*100:0;
    mlH+='<td class="money"><b>'+brl(v)+'</b><br><span style="font-size:.6rem;color:#8a7e72">'+pct.toFixed(0)+'%</span></td>';
  });
  mlH+='</tr></tbody></table>';
  const mlEl=document.getElementById('sss-marca-linha');
  if(mlEl)mlEl.innerHTML=mlH;
  ...
  LINHAS_ORD.forEach(l=>{
    ...
    lsH+='<div style="font-size:.72rem;color:#5a5047;margin-top:4px">'+brl(d.f27)+'</div>';
  ...
  ciA.forEach((c,i)=>{dh+=`<tr><td class="rk">#${i+1}</td><td><b>${c.cid}</b></td><td>${c.uf}</td><td><span class="ppill cl-${pcX(c.pfd)}">${c.pfd}</span></td><td class="num">${c.cli}</td><td class="num">${num(c.q)}</td><td class="money"><b>${brl(c.f)}</b></td></tr>`});
  ...
  ORD2.filter(p=>cpM[p]).forEach(p=>{const v=cpM[p];const pct=cpT?v.f/cpT*100:0;const pm=v.q?v.f/v.q:0;cph+=`<div class="cpcard"><div class="cpttl"><span class="ppill cl-${pcX(p)}">${p}</span></div><div class="cpbig">${brl(v.f)}</div><div class="cpsub">${pct.toFixed(1)}% do faturamento · PM ${brl(pm)}</div><div class="cpbar"><div style="width:${pct}%"></div></div><div class="cpstats"><div><b>${v.cities}</b><small>cidades</small></div><div><b>${v.cli}</b><small>clientes</small></div><div><b>${num(v.q)}</b><small>peças</small></div></div></div>`});
  ...
  MARCAS_O.forEach(m=>{let tot=0;let cells='';ORD2.filter(p=>cpM[p]).forEach(p=>{const v=bxc[m+'|'+p];const fv=v?v.f:0;tot+=fv;cells+=`<td class="money">${fv?brl(fv):'—'}</td>`});
    bxh+=`<tr><td><span class="pill pl-${mcls(m)}">${m}</span></td>${cells}<td class="money"><b>${brl(tot)}</b></td></tr>`});
  bxh+='<tr style="background:#fbf7f0;font-weight:700"><td>TOTAL</td>';
  let gt=0;
  ORD2.filter(p=>cpM[p]).forEach(p=>{const v=cpM[p].f;gt+=v;bxh+=`<td class="money"><b>${brl(v)}</b></td>`});
  bxh+=`<td class="money"><b>${brl(gt)}</b></td></tr></tbody></table>`;
  ...
  const cds=FX.map(f=>{const x=fm[f];const pmx=x.q?x.f/x.q:0;const fp=fT?x.f/fT*100:0;const sp=sT?x.sku.size/sT*100:0;const fatPorSku=x.sku.size?x.f/x.sku.size:0;const pcsPorSku=x.sku.size?x.q/x.sku.size:0;const pcsEquiv=pmx?x.f/pmx:0;const code=f==='ENTRADA'?'ENT':f==='MÉDIO'?'MED':'PRE';return `<div class="fxc ${code}"><div class="fxs">${f}</div><div class="fxn">${brl(x.f)}</div><div class="fxd">${fp.toFixed(1)}% do faturamento</div><div class="fxbar"><div style="width:${fp}%"></div></div><div class="fxstats"><div><b>${x.sku.size}</b><small>SKUs (${sp.toFixed(0)}%)</small></div><div><b>${num(x.q)}</b><small>peças</small></div><div><b>${x.cli.size}</b><small>clientes</small></div><div><b>${brl(pmx)}</b><small>PM unitário</small></div></div><div class="fxprod"><div class="fxprod-row"><span>Fat médio por SKU</span><b>${brl(fatPorSku)}</b></div><div class="fxprod-row"><span>Peças vendidas por SKU</span><b>${pcsPorSku.toFixed(1)}</b></div><div class="fxprod-row"><span>Peças equivalentes (Fat÷PM)</span><b>${num(Math.round(pcsEquiv))}</b></div></div></div>`}).join('');
  document.getElementById('fx-cards').innerHTML=`<div class="fxgrid">${cds}</div>`;

  let fxh='<table class="m"><thead><tr><th>Faixa</th><th class="r">SKUs</th><th class="r">% Mix</th><th class="r">Peças</th><th class="r">Clientes</th><th class="r">PM unitário</th><th class="r">Fat / SKU</th><th class="r">Peças / SKU</th><th class="r">Peças equiv</th><th class="r">Faturamento</th><th class="r">% Total</th><th class="r">Eficiência</th></tr></thead><tbody>';
  FX.forEach(f=>{const x=fm[f];const pmx=x.q?x.f/x.q:0;const fp=fT?x.f/fT*100:0;const sp=sT?x.sku.size/sT*100:0;const ef=sp?fp/sp:0;const ec=ef>1.2?'vu':ef<0.8?'vd':'ve';const fatPorSku=x.sku.size?x.f/x.sku.size:0;const pcsPorSku=x.sku.size?x.q/x.sku.size:0;const pcsEquiv=pmx?x.f/pmx:0;fxh+=`<tr><td><span class="fxp fxp-${fxcls(f)}">${f}</span></td><td class="num">${x.sku.size}</td><td class="num">${sp.toFixed(0)}%</td><td class="num">${num(x.q)}</td><td class="num">${x.cli.size}</td><td class="money">${brl(pmx)}</td><td class="money"><b>${brl(fatPorSku)}</b></td><td class="num"><b>${pcsPorSku.toFixed(1)}</b></td><td class="num"><b>${num(Math.round(pcsEquiv))}</b></td><td class="money"><b>${brl(x.f)}</b></td><td class="num">${fp.toFixed(1)}%</td><td class="num ${ec}"><b>${ef.toFixed(2)}×</b></td></tr>`});
  ...
  if(stSum){stSum.innerHTML=`<div class="strat-grid"><div class="strat-hero"><div class="sh-big">${brl(totFat2)}</div><div class="sh-sub">faturamento V27 do recorte · <b>${skuTot_} SKUs</b> ativos · PM <b>${brl(qtd?fat/qtd:0)}</b></div><div class="sh-split"><div class="sh-bar"><div class="sh-lbl">PREMIUM</div><div class="sh-bbox"><div class="sh-fill sh-pre" style="width:${fxPct.PREMIUM}%"></div></div><div class="sh-pct">${fxPct.PREMIUM.toFixed(0)}%</div></div><div class="sh-bar"><div class="sh-lbl">MÉDIO</div><div class="sh-bbox"><div class="sh-fill sh-med" style="width:${fxPct['MÉDIO']}%"></div></div><div class="sh-pct">${fxPct['MÉDIO'].toFixed(0)}%</div></div><div class="sh-bar"><div class="sh-lbl">ENTRADA</div><div class="sh-bbox"><div class="sh-fill sh-ent" style="width:${fxPct.ENTRADA}%"></div></div><div class="sh-pct">${fxPct.ENTRADA.toFixed(0)}%</div></div></div></div><div class="strat-actions"><div class="sa-hd">DIREÇÃO ESTRATÉGICA</div><div class="sa-item ok"><div><b>CONCENTRAÇÃO ABC</b><br><span>${skuA} SKUs Classe A (${(skuA/skuTot_*100).toFixed(0)}% do mix) sustentam ${topAPct.toFixed(0)}% do faturamento. Priorizar reposição desses itens.</span></div></div><div class="sa-item ${premShare<25?'alert':premShare>40?'ok':''}"><div><b>ARQUITETURA DE PREÇO</b><br><span>Premium = ${fxPct.PREMIUM.toFixed(0)}% do fat · ${premSKU} SKUs (${premShare.toFixed(0)}% do mix). ${premShare<25?'Ampliar linha premium.':fxPct.PREMIUM>50?'Reforçar entrada.':'Balanceado.'}</span></div></div><div class="sa-item ok"><div><b>MARCA LÍDER: ${mkTop[0]}</b><br><span>${totFat2?(mkTop[1]/totFat2*100).toFixed(0):0}% do faturamento do recorte.</span></div></div><div class="sa-item"><div><b>COBERTURA</b><br><span>${sA.filter(r=>r.cli>=6&&r.q/r.cli<=4).length} SKUs com alta cobertura e baixa profundidade — candidatos a aprofundar grade.</span></div></div></div></div>`;}

  
  // RANK POR ESTADO YoY (V26 vs V27 dos clientes recorrentes)
  const ufYoYData=UFYOY||[];
  let ufY='<table class="m"><thead><tr><th>#</th><th>UF</th><th class="r">Clientes rec.</th><th class="r">V26</th><th class="r">V27</th><th class="r">Δ R$</th><th class="r">SSS YoY</th><th class="r">Representatividade V27</th></tr></thead><tbody>';
  ...
  ufYoYData.forEach((r,i)=>{
    ...
    ufY+=`<tr><td class="rk">#${i+1}</td><td><b>${r.UF}</b></td><td class="num">${r.cli}</td><td class="money">${brl(r.V26)}</td><td class="money"><b>${brl(r.V27)}</b></td><td class="money ${delta>=0?'vu':'vd'}"><b>${delta>=0?'+':''}${brl(delta)}</b></td><td class="num ${vCl}"><b>${r.SSS>=0?'+':''}${r.SSS.toFixed(1)}%</b></td><td class="num">${rep.toFixed(1)}%</td></tr>`;
  });
  const elUfY=document.getElementById('uf-yoy');
  if(elUfY)elUfY.innerHTML=ufY+'</tbody></table>';
  ...
  let fgH='<table class="m"><thead><tr><th>Faixa PM</th><th class="r">SKUs</th><th class="r">% SKUs</th><th class="r">Peças</th><th class="r">% Qtd</th><th class="r">Fat/SKU</th><th class="r">Pç/SKU</th><th class="r">Faturamento</th><th class="r">% Fat</th></tr></thead><tbody>';
  BINS.forEach(b=>{
    ...
    fgH+=`<tr><td><b>R$ ${b[2]}</b></td><td class="num">${x.skus}</td><td class="num">${spct.toFixed(1)}%</td><td class="num">${num(x.qtd)}</td><td class="num">${qpct.toFixed(1)}%</td><td class="money">${brl(fps)}</td><td class="num">${pps.toFixed(1)}</td><td class="money"><b>${brl(x.fat)}</b></td><td class="num">${fpct.toFixed(1)}%</td></tr>`;
  });
  const elFg=document.getElementById('faixas-granular');
  if(elFg)elFg.innerHTML=fgH+'</tbody></table>';
  ...
  ['Metrópole','Grande','Média','Pequena','Micro'].forEach(p=>{
    ...
    cpH+=`<tr><td><b>${p}</b></td><td class="num">${x.cid.size}</td><td class="num">${x.cli.size}</td><td class="money">${brl(x.V26)}</td><td class="money"><b>${brl(x.V27)}</b></td><td class="money">${brl(v27rec)}</td><td class="num ${sCl}"><b>${sss>=0?'+':''}${sss.toFixed(1)}%</b></td><td class="num">${rep.toFixed(1)}%</td></tr>`;
  });
  const elCp=document.getElementById('cid-perfil-ibge');
  if(elCp)elCp.innerHTML=cpH+'</tbody></table>';

  // MOODBOARD - visual grid para time de produto - ORDENADO POR PEÇAS VENDIDAS
  // (sA mantém ordem por faturamento para curva ABC; clone só para o moodboard)
...
${chipsHtml?`<div class="mb-chips">${chipsHtml}</div>`:''}
  ...
  function renderRank(skus, targetId, showAbc=true){
    ...
    let h='<table class="t"><thead><tr><th>#</th><th>Img</th><th>SKU/Produto</th><th>Marca</th><th>Faixa</th>'+(showAbc?'<th>ABC</th>':'')+'<th class="r">Pç</th><th class="r">Cli</th><th class="r">PM</th><th class="r">Faturamento</th><th class="r">% Acum</th></tr></thead><tbody>';
    skus.forEach((r,i)=>{
      const cp=r.co&&r.co!=='SEM COORDENADO'?`<span class="cpill">${r.co}</span>`:'';
      h+=`<tr><td class="rk">#${i+1}</td><td>${img(r.p,38)}</td><td><div class="sku">${r.p}</div>${r.est?`<div style="font-size:.58rem;color:#8b6a8a;font-weight:600">${r.est}</div>`:''}<div class="pnm">${r.dp} ${cp}</div></td><td><span class="pill pl-${mcls(r.m)}">${r.m}</span></td><td><span class="fxp fxp-${fxcls(r.fx)}">${r.fx}</span></td>${showAbc?`<td><span class="abc abc-${r.ABC}">${r.ABC}</span></td>`:''}<td class="num">${num(r.q)}</td><td class="num">${r.cli}</td><td class="money">${brl(r.pm)}</td><td class="money"><b>${brl(r.f)}</b></td><td class="num">${(r.acum||0).toFixed(1)}%</td></tr>`;
    });
  ...
  document.getElementById('coord').innerHTML=coA.map(c=>`<div class="ccc"><div class="ccn">◆ ${c.co}</div><div class="ccm"><b>${brl(c.f)}</b> · ${num(c.q)}pç · ${c.sku}SKU · ${c.cli}cli</div></div>`).join('');
  ...
  let mxh='<table class="m"><thead><tr><th>Tipo de Produto</th><th class="r">SKUs</th><th class="r">% Mix</th><th class="r">Peças</th><th class="r">Cli</th><th class="r">PM</th><th class="r">Faturamento</th><th class="r">% Fat</th><th class="r">Eficiência</th><th>Recomendação</th></tr></thead><tbody>';
  mixA.forEach(g=>{const ec=g.ef>1.3?'vu':g.ef<0.7?'vd':'ve';const rec=g.ef>1.3?'APROFUNDAR':g.ef<0.7?'RACIONALIZAR':'MANTER';const rcls=g.ef>1.3?'ok':g.ef<0.7?'alert':'';const efBar=Math.min(g.ef*50,100);const efCol=g.ef>1.3?'#4a8b5a':g.ef<0.7?'#c94a2a':'#b88a3a';mxh+=`<tr><td><b>${g.g}</b></td><td class="num">${g.sku}</td><td class="num">${g.sp.toFixed(0)}%</td><td class="num">${num(g.q)}</td><td class="num">${g.cli}</td><td class="money">${brl(g.pm)}</td><td class="money"><b>${brl(g.f)}</b></td><td class="num">${g.fp.toFixed(1)}%</td><td class="num ${ec}"><div style="display:flex;align-items:center;gap:6px;justify-content:flex-end"><div style="width:60px;height:6px;background:#f0ebe3;border-radius:3px;overflow:hidden"><div style="width:${efBar}%;height:100%;background:${efCol}"></div></div><b>${g.ef.toFixed(2)}×</b></div></td><td><span class="rec ${rcls}">${rec}</span></td></tr>`});
  document.getElementById('mix-opt').innerHTML=mxh+'</tbody></table>';

  // Insights
  // insights tables without margem
  const tblI=rs=>{if(!rs.length)return '<div class="empty">—</div>';let h='<table class="m"><thead><tr><th>Img</th><th>SKU</th><th>Produto</th><th>Marca</th><th>Faixa</th><th class="r">Pç</th><th class="r">Cli</th><th class="r">Fat</th></tr></thead><tbody>';rs.forEach(r=>{h+=`<tr><td>${img(r.p,32)}</td><td class="sku">${r.p}${r.est?`<br><span style="font-size:.55rem;color:#8b6a8a">${r.est}</span>`:''}</td><td>${r.dp}</td><td><span class="pill pl-${mcls(r.m)}">${r.m.substring(0,4)}</span></td><td><span class="fxp fxp-${fxcls(r.fx)}">${r.fx}</span></td><td class="num">${num(r.q)}</td><td class="num">${r.cli}</td><td class="money"><b>${brl(r.f)}</b></td></tr>`});return h+'</tbody></table>'};
